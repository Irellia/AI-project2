...
class Model(object):
    ...
    def __init__(self, restore=None):
        ...
        self.x = tfv1.placeholder(tf.float32, shape=[None, 373])
        self._y = tfv1.placeholder(tf.float32, shape=[None, 1])

        # Output
        self.y = self.buildNetwork(self.x)

        # # Build the optimizer
        # self.cost = tf.reduce_mean(tf.abs(tf.raw_ops.Sub(self.y, self._y)))
        # self.optimizer = tfv1.train.AdamOptimizer().minimize(self.cost)
        #
        # # Add ops to save and restore all the variables.
        # self.saver = tfv1.train.Saver()
        # # Operation to initialize the variables
        # self.init = tfv1.initialize_all_variables()
        #
        # # Start session
    ...
    def buildNetwork(self, x):
        ...
        globalW = tf.Variable(tfv1.random_normal([37, 20], stddev=0.35),
                              name="globalW")
        globalB = tf.Variable(tfv1.random_normal([20], stddev=0.35),
                              name="globalB")
        pieceW = tf.Variable(tfv1.random_normal([208, 100], stddev=0.35),
                             name="pieceW")
        pieceB = tf.Variable(tfv1.random_normal([100], stddev=0.35),
                             name="pieceB")
        squareW = tf.Variable(tfv1.random_normal([128, 50], stddev=0.35),
                              name="squareW")
        squareB = tf.Variable(tfv1.random_normal([50], stddev=0.35),
                              name="squareB")

        # Weights and biases for the second hidden layer
        w2 = tf.Variable(tfv1.random_normal([170, 75], stddev=0.35),
                         name="W2")
        b2 = tf.Variable(tfv1.random_normal([75], stddev=0.35),
                         name="B2")

        # Weights and biases for the output layer
        w3 = tf.Variable(tfv1.random_normal([75, 1], stddev=0.35),
                         name="W3")
        b3 = tf.Variable(tfv1.random_normal([1], stddev=0.35),
                         name="B3")
    ...
    def bootstrap(self, trainDB=None, testDB=None, batchSize=1000, epochs=50, displayStep=1):
            ...
            while iteration < iPerE:
                ...
                if epoch % displayStep == 0 and iteration == 0:
                    batchError = self.sess.run(self.cost, feed_dict={self.x: batch_xs,
                                                                     self._y: batch_ys})
                    print
                    "Epoch ", epoch, " - Minibatch Avg Error " + "{:.6f}".format(
                        batchError)
        ...
        print("Testing Error:", self.sess.run(self.cost,
               feed_dict={self.x: testDB.getFeats(), self._y: testDB.getLabels()}))
...
class NeuralNet(player.Player):

    def __init__(self, depth=3, model=None):
        player.Player.__init__(self, 'white')
        self.model = model
        if model is None:
